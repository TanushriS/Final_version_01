...
const Dashboard = () => {
  const { 
    ...
    healthData, 
    onRetryBattery, 
    onFetchWeather } = useContext(ThermoSenseContext);
      ...
      <div className="dashboard-header">
        ...
        <div className="status-badges">
          <div className={`status status--${healthData.alertLevel === "safe" ? "success" : healthData.alertLevel === "warning" ? "warning" : "error"}`}>
            {healthData.alertLevel === "safe" ? "System Operational" : healthData.alertLevel === "warning" ? "Monitor Required" : "Immediate Action Required"}
          </div>
      ...
      <div className="dashboard-grid">
        ...
        <div className="widget card temperature-widget">
          ...
          <div className="temperature-display">
            ...
            <div className="temp-status">{deviceTemp > 40 ? "Critical temperature" : deviceTemp > 35 ? "Elevated temperature" : "Normal range"}</div>
          </div>
        </div>

        <WeatherWidget weatherData={weatherData} onRetry={onFetchWeather} />
        ...
        <div className="widget card alerts-widget">
          ...
          <div className="alerts-list">
            {healthData.alertLevel === "danger" ? (
              <div className="alert-item critical">🚨 Critical temperature detected - immediate action required!</div>
            ) : healthData.alertLevel === "warning" ? (
              <div className="alert-item warning">⚠️ Device temperature elevated - monitor closely</div>
            ) : (
